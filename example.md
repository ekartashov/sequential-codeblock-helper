# Sequential Code Blocks Guide

This document demonstrates the use of sequential code blocks in markdown with variable dependencies between blocks.

## Overview

Sequential code blocks allow you to:

1. Break down complex operations into logical steps
2. Maintain variable state between blocks
3. Run blocks independently using preset variables
4. Document the process while keeping executable commands
...
## Advanced Usage with Conditional Execution
...
## Complex Variable Dependencies and Error Handling
...
## Variables
ZSH_THEMES_DIR="${MOUNT2}/usr/share/zsh/themes"
SELECTED_THEME="default"

## Script
# This demonstrates a ZSH-specific block that depends on previous blocks
echo "Setting up ZSH themes..." | tee -a "$LOG_FILE"

mkdir -p "$ZSH_THEMES_DIR"

# Create a sample theme file
cat > "${ZSH_THEMES_DIR}/${SELECTED_THEME}.zsh-theme" << 'EOF'
# Default ZSH theme
...
if [ -f "${SHELL_CONFIG_DIR}/.zshrc" ]; then
  echo "Updating .zshrc to use the ${SELECTED_THEME} theme" | tee -a "$LOG_FILE"
  cat >> "${SHELL_CONFIG_DIR}/.zshrc" << EOF

# Load theme
if [[ -f /usr/share/zsh/themes/${SELECTED_THEME}.zsh-theme ]]; then
  source /usr/share/zsh/themes/${SELECTED_THEME}.zsh-theme
fi
EOF
fi
```

## Nested Directory and Permissions Setup
...
## Final Validation with Nested Dependencies
...
## Special Case: Indented Blocks

- Indented example in a list showing another language:
  ```python <!-- SEQUENTIAL_CODEBLOCK -->
  ## Preset Variables
  # MOUNT2="/mnt/root"
  # APP_DIR="${MOUNT2}/opt/myapp"
  ...
  config_path = f"{APP_DIR}/config/app.conf"
  output_path = f"{APP_DIR}/config/app_config.json"
  
  ## Script
  # Python script to convert the app.conf to JSON
  ...
  print(f"Converted {config_path} to JSON: {output_path}")
  ```

> Note block with a sequential codeblock:
>
> ```sh <!-- SEQUENTIAL_CODEBLOCK -->
> ## Preset Variables
> # MOUNT2="/mnt/root"
> # SUMMARY_FILE="${MOUNT2}/installation_summary.txt"
> 
> ## Variables
> DEST="/var/log/installation_$(date +%Y%m%d).log"
> 
> ## Script
> # Copy the summary file to a permanent location
> if [ -f "$SUMMARY_FILE" ]; then
>   echo "Copying summary to permanent log at $DEST"
>   cp "$SUMMARY_FILE" "$DEST"
>   echo "Summary saved to $DEST"
> else
